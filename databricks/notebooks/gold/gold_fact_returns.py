"""Gold fact table for fact_returns — Databricks notebook entrypoint.

Reads from silver.returns, silver.order_items, silver.orders, and the four
conformed Gold dimensions (dim_date, dim_channel, dim_customer, dim_product),
resolves dimension foreign keys via the reusable module helper (including SCD2
point-in-time joins for customer and product), and writes the final
gold.fact_returns Delta table.
...
def get_spark() -> SparkSession:
    ...
    return SparkSession.builder.appName("gold_fact_returns").getOrCreate()


def read_silver_returns(spark: SparkSession) -> DataFrame:
    """Read the current Silver returns table."""
    return spark.table(SILVER_RETURNS_TABLE)
...
def write_gold_fact_returns(df: DataFrame) -> None:
    """Write the Gold fact_returns dataset as a Delta table."""
    (
...
def main() -> None:
    """Run the Gold fact_returns build."""
    spark = get_spark()

    gold_df = build_fact_returns(
        silver_returns_df=read_silver_returns(spark),
    ...
    write_gold_fact_returns(gold_df)
